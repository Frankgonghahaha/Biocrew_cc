# =============================================
# Evaluate_pipeline_5.py — 功能与使用说明（中文）
# =============================================
# 【主要功能】
...
# 【使用方式（一页纸）】
# 1) 准备
# - 模型目录：`--model-dir` 指向包含多个 SBML（.xml/.sbml）的目录；文件名（去扩展名）作为成员 ID。
# - 培养基表：`--medium-csv` 至少包含两列：reaction；flux 或 suggested_upper_bound（脚本统一为 upper，正数=最大摄入上界）。
# - 脚本会强制追加/提升 EX_dbp_m=20 到培养基，并用 MICOM 的 community.medium 应用到社区。
#   依赖：Python ≥ 3.8，micom，cobra，pandas。
#
# 2) 基本运行（Baseline）
#   python Evaluate_pipeline_4.py \
#     --model-dir /path/to/models \
#     --medium-csv /path/to/medium.csv \
#     --alpha 0.7
# 输出：Result6_community.xlsx（默认写入 --model-dir）
#  · Simulate result：整体社区阶段二的 Biomass（群落增长率）与 DBP flux（EX_dbp_m 固定通量，负值=摄入）
#  · Microbial growth：阶段二各成员增长率（若可获取）
#
# 3) 指定输出路径
#   python Evaluate_pipeline_4.py --model-dir M --medium-csv MED.csv --alpha 0.7 \
#     --out /path/to/Result6_community.xlsx
#
# 4) α 扫描（Alpha Scan）
#   # 扫描默认 α 集合 0.5,0.6,0.7,0.8,0.9
#   python Evaluate_pipeline_4.py --model-dir M --medium-csv MED.csv --alpha-scan
#   # 自定义 α 列表
#   python Evaluate_pipeline_4.py --model-dir M --medium-csv MED.csv --alpha-scan --alphas 0.55,0.7,0.9
# 额外生成：Result6_community_alpha_scan.csv（写到 --model-dir）
#
# 5) 稳健性分析（Robust：逐一去除物种）
#   python Evaluate_pipeline_4.py --model-dir M --medium-csv MED.csv --alpha 0.7 --Robust
# Excel 新增：Robust_Summary（列：Removal species、Biomass、tradeoff value、group growth rate、DBP flux、
# group growth change、DBP flux change、group growth change percentage、DBP flux change percentage）；
# 并保留兼容旧脚本的 Robust 表。
#
# 6) 组合模拟（Group：按规模做 k 组合）
#   python Evaluate_pipeline_4.py --model-dir M --medium-csv MED.csv --alpha 0.7 \
#     --group --min_number 2 --max_number 4
# Excel 新增：Group_Summary（列：Members、group number、Biomass、tradeoff value、group growth rate、DBP flux）；
# 并保留兼容旧脚本的 Group 表。
#
# 6.1) 组合模拟（Group CSV）+ 稳健性（Robust）
#   # 单次运行同时：
#   #   · 按 CSV（members/consortium_id）逐组模拟（从 --model-dir 载入同名 .xml）
#   #   · 对“基线社区”（全部成员）做一次稳健性分析（顺序去除单个成员）
#   python Evaluate_pipeline_5.py \
#     --model-dir "/Users/frank_gong/文档/生物智能体/20251015/20251103/添加路径_代谢模型" \
#     --medium-csv "/Users/frank_gong/文档/生物智能体/20251015/20251103/recommended_medium.csv" \
#     --alpha 0.7 \
#     --group-csv "/Users/frank_gong/文档/生物智能体/20251015/20251103/Result4_optimal_consortia_ASMAG.csv" \
#     --Robust \
#     --out "/Users/frank_gong/文档/生物智能体/20251015/20251103/Result6_community.xlsx"
#
#   说明：
#   - CSV 必填列：members（或 Members）；可选列：consortium_id（作为组名，输出 Group_Summary 的第一列）
#   - 成员名需与 --model-dir 内 SBML 文件名（去扩展名）一致；缺失的成员会被跳过并在日志中提示
#   - 运行日志会显示：正在进行<consortium_id>的模拟
#   - 输出包含：
#       · Group_Summary：Group, Members, group number, Biomass, tradeoff value, group growth rate, DBP flux
#       · Group（兼容表）
#       · Robust_Summary：Removal species, Biomass, tradeoff value, group growth rate, DBP flux,
#                         group growth change, DBP flux change, group growth change percentage, DBP flux change percentage
#       · Robust（兼容表）
#
# 7) 常见提示
# - 若社区中不存在 EX_dbp_m：会提示并返回 NaN 的摄入值。
# - 若 Biomass_max=0：阶段二会跳过（说明培养基或模型不可行）。
# - 数值钳制：阶段二的 group growth rate 会被钳在 Biomass_max 附近，避免数值抖动看似超过最大值。
# -*- coding: utf-8 -*-
...
def main():
    ...
    parser.add_argument("--group-csv", type=str, help="按CSV指定的群落成员进行模拟（列：Members 或 members，';'分隔）")
    ...
    print("  group_csv  =", getattr(args, "group_csv", None))
    print("  group_mode =", ("CSV" if getattr(args, "group_csv", None) else ("enumeration" if args.group else "off")))
    ...
    try:
                    ...
                    for combo in combinations(taxa_ids, k):
                        ...
                        try:
                            ...
                            group_results.append({
                                "k": k,
                                "Members": ",".join(combo),
                                # New fields for Group_Summary
                                "Biomass_max": biomass_max_k,            # 阶段一最大群落生长
                                "group_growth_rate": biomass_stage2_k,   # 阶段二群落增长（原先的“Biomass”）
                                "DBP flux": dbp_flux_k,                  # 阶段二固定 EX_dbp_m
                                "tradeoff value": args.alpha,            # α
                                "group number": k                        # 组规模 k
                            })
                        except Exception as e:
                            group_results.append({
                                "k": k,
                                "Members": ",".join(combo),
                                "Biomass_max": float("nan"),
                                "group_growth_rate": float("nan"),
                                "DBP flux": float("nan"),
                                "tradeoff value": args.alpha,
                                "group number": k
                            })
                    print(f"[Group] k={k} 组合数：{cnt}")

        # ---------- Group simulation from CSV (explicit groups) ----------
        if getattr(args, "group_csv", None):
            import pandas as _pd
            import re as _re

            def _split_members_text(s: str):
                if s is None:
                    return []
                parts = [p.strip() for p in _re.split(r"[;；,，、|]", str(s)) if str(p).strip()]
                return parts

            csv_path = os.path.abspath(args.group_csv)
            if not os.path.exists(csv_path):
                print(f"[GroupCSV] 文件不存在：{csv_path}")
            else:
                try:
                    gdf = _pd.read_csv(csv_path)
                except Exception as e:
                    print(f"[GroupCSV] 读取失败：{csv_path} — {e}")
                    gdf = None
                if gdf is not None and not gdf.empty:
                    # 兼容列名：members 或 Members
                    col_members = None
                    for cand in ["members", "Members"]:
                        if cand in gdf.columns:
                            col_members = cand
                            break
                    if col_members is None:
                        print(f"[GroupCSV] 未找到成员列 'members' 或 'Members'，可用列：{list(gdf.columns)}")
                    else:
                        taxa_ids = tax_base["id"].astype(str).tolist() if isinstance(tax_base, pd.DataFrame) and "id" in tax_base.columns else []
                        taxa_set = set(taxa_ids)
                        print(f"[GroupCSV] 读取 {len(gdf)} 个 group；可用模型 {len(taxa_set)} 个。")
                        for idx, row in gdf.iterrows():
                            # 读取组名：优先使用 consortium_id，其次 group_id/Group/group/id；若缺失则用行号占位
                            consortium_col = None
                            for _cand in ["consortium_id", "group_id", "Group", "group", "id"]:
                                if _cand in gdf.columns:
                                    consortium_col = _cand
                                    break
                            _raw_label = str(row.get(consortium_col)) if consortium_col else ""
                            group_label = _raw_label.strip()
                            if not group_label or group_label.lower() == "nan":
                                group_label = f"group_{idx+1}"
                            members_text = row.get(col_members, "")
                            combo = _split_members_text(members_text)
                            if not combo:
                                continue
                            # 过滤缺失成员，仅保留存在于 taxa 的
                            combo_valid = [m for m in combo if str(m) in taxa_set]
                            missing = [m for m in combo if str(m) not in taxa_set]
                            if missing:
                                print(f"  [GroupCSV] 行{idx}: 缺失模型 {missing} — 已跳过这些成员（其余照常模拟）")
                            if group_label:
                                print(f"  [GroupCSV] 组名: {group_label}")
                            if not combo_valid:
                                print(f"  [GroupCSV] 行{idx}: 有效成员为空，跳过。")
                                continue
                            k = len(combo_valid)
                            taxa_new = tax_base[tax_base["id"].astype(str).isin(combo_valid)].copy()
                            print(f"正在模拟{group_label}的群落模拟")
                            try:
                                com_k = Community(taxa_new, name=f"COMM_GROUP_CSV_k{k}")
                                apply_medium_via_micom(com_k, medium)
                                biomass_max_k = step1_max_growth(com_k)
                                if biomass_max_k > 1e-12:
                                    stage2_growth_k, dbp_flux_k, _ = step2_max_dbp_uptake(com_k, args.alpha, biomass_max_k)
                                ...
                                group_results.append({
                                    "k": k,
                                    "Members": ",".join(combo_valid),
                                    "Biomass_max": biomass_max_k,
                                    "group_growth_rate": biomass_stage2_k,
                                    "DBP flux": dbp_flux_k,
                                    "tradeoff value": args.alpha,
                                    "group number": k,
                                    "Group": group_label,
            ...
            if not isinstance(tax_base, pd.DataFrame) or tax_base.empty or "id" not in tax_base.columns:
                ...
                for remove_id in taxa_ids:
                    ...
                    try:
                        ...
                        robust_results.append({
                            "Removal species": remove_id,
                            "Biomass_max": biomass_max_rb,             # 阶段一最大群落生长
                            "group_growth_rate": biomass_stage2_rb,    # 阶段二群落增长
        ...
        with pd.ExcelWriter(out_path) as writer:
            ...
            # ----- New: Group_Summary sheet with renamed columns -----
            if (args.group or getattr(args, "group_csv", None)) and group_results:
                _gdf = pd.DataFrame(group_results)
                # 优先以 CSV 中的组名作为第一列（列名统一显示为 "Group"）
                sel_cols = []
                if "Group" in _gdf.columns:
                    sel_cols.append("Group")
                elif "consortium_id" in _gdf.columns:
                    sel_cols.append("consortium_id")
                sel_cols += [
                    "Members",
                    "group number",
                    "Biomass_max",
                ...
                # 兼容：缺哪列就造空列
                for c in sel_cols:
                    if c not in _gdf.columns:
                        _gdf[c] = float("nan")
                group_summary = _gdf[sel_cols].rename(columns={
                    "Biomass_max": "Biomass",
                    "group_growth_rate": "group growth rate",
                    "consortium_id": "Group"
                })
                group_summary.to_excel(writer, index=False, sheet_name="Group_Summary")

                # Legacy sheet (kept for backward compatibility)
                try:
                    _legacy_dict = {
                        "k": _gdf.get("k", _gdf.get("group number")),
                        "Members": _gdf["Members"],
                        "Biomass": _gdf["group growth rate" if "group growth rate" in group_summary.columns else "group_growth_rate"],
                        "DBP flux": _gdf["DBP flux"],
                    }
                    if "Group" in _gdf.columns:
                        _legacy_dict = {"Group": _gdf["Group"], **_legacy_dict}
                    elif "consortium_id" in _gdf.columns:
                        _legacy_dict = {"Group": _gdf["consortium_id"], **_legacy_dict}
                    group_legacy = pd.DataFrame(_legacy_dict)
                    group_legacy.to_excel(writer, index=False, sheet_name="Group")
                except Exception:
                    pass

            # ----- Robust_Summary and legacy Robust sheet -----
            if args.Robust:
                rdf = pd.DataFrame(robust_results)
                # Ensure columns exist
                for c in ["Removal species", "Biomass_max", "group_growth_rate", "DBP flux", "tradeoff value"]:
                    if c not in rdf.columns:
                        rdf[c] = float("nan")

                # Compute baseline deltas vs. baseline stage-2 (biomass_stage2) and dbp_flux
                try:
                    _baseline_growth = float(biomass_stage2)
                except Exception:
                    _baseline_growth = float("nan")
                try:
                    _baseline_dbp = float(dbp_flux)
                except Exception:
                    _baseline_dbp = float("nan")
                ...
                rdf["group growth change"] = rdf["group_growth_rate"] - _baseline_growth
                rdf["DBP flux change"] = rdf["DBP flux"] - _baseline_dbp
                rdf["group growth change percentage"] = rdf.apply(lambda x: _pct_change(x["group_growth_rate"], _baseline_growth), axis=1)
                rdf["DBP flux change percentage"] = rdf.apply(lambda x: _pct_change(x["DBP flux"], _baseline_dbp), axis=1)
                ...
                robust_summary = rdf[[
                    ...
                    "group growth change",
                    "DBP flux change",
                    "group growth change percentage",
                    "DBP flux change percentage",
                ]].rename(columns={
                    "Biomass_max": "Biomass",
                    "group_growth_rate": "group growth rate"
                })

                robust_summary.to_excel(writer, index=False, sheet_name="Robust_Summary")

                # Legacy sheet for backward compatibility
                try:
                    robust_legacy = pd.DataFrame({
                        "Removal species": rdf["Removal species"],
                        "DBP flux": rdf["DBP flux"],
                        # 旧版“Biomass”列沿用阶段二增长以兼容原有外部脚本
                        "Biomass": rdf["group_growth_rate"],
                    })
                    robust_legacy.to_excel(writer, index=False, sheet_name="Robust")
                except Exception:
                    pass
