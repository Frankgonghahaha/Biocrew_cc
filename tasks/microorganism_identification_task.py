...
class MicroorganismIdentificationTask:
    ...
    def create_task(self, agent, context_task=None, feedback=None, user_requirement=None):
        ...
        # 5. 使用NCBI基因组查询工具获取微生物基因组信息
        #    - NCBIGenomeQueryTool使用方式：ncbi_genome_query_tool._run({"organism_name": "微生物名称"})
        #    - 记录每个推荐微生物的Assembly Accession编号
        # 6. 综合分析所有工具返回的结果，形成完整的代谢网络图
        # 7. 在最终报告中明确体现各工具查询到的具体信息和综合分析结论
        # 8. 提供数据完整性和可信度的评估，明确指出哪些数据可用，哪些数据缺失
        # 9. 对于每个识别的微生物，需要判断是否可获取全基因组信息
        # 10. 评估微生物的生长环境要求（温度、pH值、盐度等）
        # 11. 分析微生物对目标污染物的降解效果
        # 12. 分析微生物间的代谢互补性，计算竞争指数和互补指数
        
        # 微生物名称精确度要求：
        # 1. 所有推荐的微生物必须精确到种（species level）
        # 2. 微生物名称必须使用标准的双名法命名，即"属名 + 种加词"，如"Pseudomonas putida"
        # 3. 禁止使用模糊的分类级别，如仅属名"Rhodococcus sp."或更高分类级别
        # 4. 如果数据库返回的是菌株信息，需要提取其种名，例如从"Rhodococcus jostii RHA1"中提取"Rhodococcus jostii"
        # 5. 必须提供完整的科学分类信息，包括：界、门、纲、目、科、属、种
        # 6. 在输出表格中，"微生物名称"列应仅包含精确到种的名称，完整分类信息可在其他列中提供
        
        # 全基因组信息获取判断：
        # 1. 通过NCBI基因组查询工具获取Assembly Accession编号
        ...
        10. 代谢互补性分析（对推荐的微生物组合，提供竞争指数和互补指数）
        11. 每个推荐微生物的NCBI Assembly Accession编号
        12. 功能微生物与代谢互补微生物的推荐组合表格，包含竞争指数和互补指数
        13. 所有推荐微生物必须精确到种，并提供完整科学分类信息
        """
