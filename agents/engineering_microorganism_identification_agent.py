...
class EngineeringMicroorganismIdentificationAgent:
    ...
    def create_agent(self):
        ...
        return Agent(
            ...
            # - Action Input: {"pollutant_name": "翻译后的标准污染物名称", "data_type": "both"}
            # - 或者使用其他专门工具如GeneDataQueryTool, OrganismDataQueryTool等
            # - 注意：必须使用双引号，不能使用单引号
            # - 注意：pollutant_name参数是必填的，且必须是翻译后的标准科学术语
            
            # 重要提醒：
            # 1. 必须严格按照上述格式调用工具
            # 2. pollutant_name参数是必填的，必须明确指定翻译后的标准污染物名称
            # 3. 根据需要选择合适的工具进行调用
            # 4. 必须从用户需求中准确识别目标污染物并进行正确翻译
            # 5. 调用工具前先思考需要查询的具体污染物名称和数据类型
            # 6. 在报告中同时提供原始用户输入中的污染物描述和翻译后的标准科学术语
            
            # 工作流程：
            # 1. 接收用户输入的水质净化目标
            # 2. 识别目标污染物并将其翻译为标准科学术语
            # 3. 查询相关基因和微生物数据
            # 4. 分析微生物的降解能力和代谢途径
            # 5. 筛选功能微生物和代谢互补微生物
            # 6. 提供完整的微生物组推荐方案
            ...
            # - 提供具体的微生物名称和基因信息
            # - 当数据缺失时明确指出并提供替代方案
            # - 在报告开头明确说明识别的原始污染物描述和翻译后的标准科学术语
            """,
            tools=tools,
            verbose=True,
