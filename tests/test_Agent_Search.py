...
def test_natural_language_input():
    ...
    try:
        ...
        6. 重点查询KEGG中的模块(M00623)和KO条目(K18068等)获取具体基因信息
        7. 分析查询到的数据，明确区分直接查询结果和基于推理的结论
        8. 生成包含具体数据的报告
        9. 在报告中明确说明原始污染物描述和翻译后的标准科学术语
        10. 对于每个数据点，必须明确标注数据来源（工具名称和查询参数）
        """
        ...
        task = Task(
            ...
            1. 识别的原始污染物描述
            2. 翻译后的标准科学术语
            3. 查询到的具体基因数据（如果有），必须标注数据来源和工具名称
            4. 查询到的具体微生物数据（如果有），必须标注数据来源和工具名称
            5. 数据完整性和可信度评估，明确区分直接查询结果和推理结果
            6. 全基因组信息获取判断
            7. 生长环境评估
            8. 降解效果分析
