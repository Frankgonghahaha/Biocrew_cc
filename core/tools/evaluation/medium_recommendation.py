...
class MediumRecommendationTool(BaseTool):
    ...
    def _run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用项目统一模型目录
            try:
                from config.paths import MODELS_DIR as unified_models_dir
            except ImportError:
                # 如果无法导入配置，使用默认路径
                project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
                unified_models_dir = os.path.join(project_root, "outputs", "metabolic_models")
            
            if os.path.exists(unified_models_dir):
                models_path = unified_models_dir
                logger.info(f"使用项目统一模型目录: {models_path}")
    ...
    def _parse_candidate_ex(self, candidate_ex: Optional[List[str]]) -> pd.DataFrame:
        ...
        if not cand_pairs:
            # 使用模型中实际存在的交换反应作为候选
            cand_pairs = [("EX_glc__D_e", 10.0), ("EX_ac_e", 5.0)]
