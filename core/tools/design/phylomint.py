...
class PhylomintTool(BaseTool):
    ...
    def _run(self, output_path: str, phylo_path: Optional[str] = None, models_path: Optional[str] = None, 
        ...
        Returns:
            dict: 分析结果
        """
        print(f"[PhylomintTool] 正在调用工具，参数: output_path={output_path}, phylo_path={phylo_path}, models_path={models_path}, function_species_csv={function_species_csv}, skip_preprocess={skip_preprocess}")
        
        # 获取项目根目录
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.join(current_dir, '..', '..')
        
        # 如果models_path未指定，则使用项目内默认路径
        if not models_path:
        ...
        try:
            ...
            # 构建命令，使用项目内部的脚本
            phylomint_script = os.path.join(project_root, 'core', 'tools', 'external', 'run_phylomint.py')
            
            # 检查项目内部的脚本是否存在
            if not os.path.exists(phylomint_script):
                # 如果项目内部脚本不存在，回退到模拟执行
                print(f"[PhylomintTool] 项目内部脚本不存在，使用模拟执行")
            ...
            # 执行命令
            print(f"[PhylomintTool] 执行命令: {' '.join(cmd)}")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)  # 5分钟超时
            
            if result.returncode != 0:
                # 如果执行失败，返回模拟结果
                print(f"[PhylomintTool] 命令执行失败，使用模拟结果，错误信息: {result.stderr}")
                return self._simulate_phylomint(output_path, phylo_path, models_path, function_species_csv, skip_preprocess)
            
            # 检查输出文件是否存在
            if not os.path.exists(output_path):
                # 如果输出文件不存在，返回模拟结果
                print(f"[PhylomintTool] 输出文件不存在，使用模拟结果")
                return self._simulate_phylomint(output_path, phylo_path, models_path, function_species_csv, skip_preprocess)
            ...
            print(f"[PhylomintTool] 工具调用成功，返回结果")
            return {
                "status": "success", 
                "data": {
            ...
            # 超时情况下返回模拟结果
            print(f"[PhylomintTool] 工具调用超时，使用模拟结果")
            return self._simulate_phylomint(output_path, phylo_path, models_path, function_species_csv, skip_preprocess)
        except Exception as e:
            # 其他异常情况下返回模拟结果
            print(f"[PhylomintTool] 工具调用出错，使用模拟结果: {str(e)}")
            return self._simulate_phylomint(output_path, phylo_path, models_path, function_species_csv, skip_preprocess)
