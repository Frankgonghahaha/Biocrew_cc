...
class Genome:
    ...
    def __init__(
        self,
        contig_filepath: str = None,
        protein_filepath: str = None,
        contig_data: Union[str, TextIO, BinaryIO] = None,
        protein_data: Union[str, TextIO, BinaryIO] = None,
        subsample: float = 1.0,
    ):
        # 支持文件路径或数据流
        self.fna_filepath = str(contig_filepath) if contig_filepath else None
        self.faa_filepath = str(protein_filepath) if protein_filepath else None
        self.contig_data = contig_data
        self.protein_data_input = protein_data
    ...
    def protein_data(self):
        ...
        if self._protein_data is None:
            ...
            # 处理蛋白质数据 - 支持文件路径或数据流
            if self.protein_data_input:
                # 如果提供了数据流或字符串数据
                if isinstance(self.protein_data_input, str):
                    # 字符串数据
                    fh = io.StringIO(self.protein_data_input)
                else:
                    # 文件对象或BytesIO对象
                    fh = self.protein_data_input
            else:
                # 从文件读取
            ...
            # 只有当不是字符串数据时才关闭文件
            if not isinstance(self.protein_data_input, str):
                fh.close()
    ...
    def compute_dna_statistics(self):
        """
        Measures nucleotide composition metrics from contigs.

        If subsampling is used, a random slice of the contigs
        is taken to reduce runtime. If contigs are small, N's are
        added to ensure the sample size is sufficient for computing
        di-nucleotide metrics.
        """
        genome_statistics = {}

        # Process contigs - 支持文件路径或数据流
        if self.contig_data:
            # 如果提供了数据流或字符串数据
            if isinstance(self.contig_data, str):
                # 字符串数据
                fh = io.StringIO(self.contig_data)
            else:
                # 文件对象或BytesIO对象
                fh = self.contig_data
        else:
        ...
        genome = ""
        for _, sequence in iterate_fasta(fh):
            if self.subsample < 1.0:
                len_sequence = len(sequence)
                len_sample = int(len_sequence * self.subsample)
                left_index = np.random.randint(0, len_sequence - len_sample)
                sequence = sequence[left_index : (len_sample + left_index)]

            genome += "NN" + sequence
        
        # 只有当不是字符串数据时才关闭文件
        if not isinstance(self.contig_data, str):
            fh.close()
...
def measure_genome_features(faa_path: str = None, fna_path: str = None, 
                           faa_data: Union[str, TextIO, BinaryIO] = None, 
                           fna_data: Union[str, TextIO, BinaryIO] = None,
                           subsample=1.0) -> Dict[str, dict]:
    """Measure features from the provided genome files or data"""
    if faa_path or fna_path:
        logging.info("Measuring features from:\n\t%s\n\t%s", fna_path, faa_path)
    else:
        logging.info("Measuring features from provided data")
        
    genome_calc = Genome(
        contig_filepath=fna_path,
        protein_filepath=faa_path,
        contig_data=fna_data,
        protein_data=faa_data,
        subsample=subsample,
    )
